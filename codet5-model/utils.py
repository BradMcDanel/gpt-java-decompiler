...
def load_and_cache_multi_gen_data(args, pool, tokenizer, split_tag, only_src=False, is_sample=False):
    ...
    if os.path.exists(cache_fn) and not is_sample:
        ...
        task_list = ['summarize', 'translate', 'refine', 'concode', 'defect', 'compilation', 'decompilation']
        for task in task_list:
            ...
            for sub_task in sub_tasks:
                ...
                elif task == 'translate' or task == 'compilation' or task == 'decompilation' :
...
def get_filenames(data_root, task, sub_task, split=''):
    if task == 'concode':
        ...
        data_dir = '{}/{}'.format(data_root, task)
        train_fn = '{}/train_datafile.jsonl'.format(data_dir)
        dev_fn = '{}/valid_datafile.jsonl'.format(data_dir)
        test_fn = '{}/test_datafile.jsonl'.format(data_dir)
    elif task == 'decompilation':
        data_dir = '{}/{}'.format(data_root, task)
        train_fn = '{}/train_datafile.jsonl'.format(data_dir)
        dev_fn = '{}/valid_datafile.jsonl'.format(data_dir)
        test_fn = '{}/test_datafile.jsonl'.format(data_dir)
...
def read_examples(filename, data_num, task):
    read_example_dict = {
        ...
        'compilation': read_java_raw_examples,
        'decompilation':read_java_compiled_examples,
