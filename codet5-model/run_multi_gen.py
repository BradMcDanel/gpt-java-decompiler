...
def get_bs(cur_task, model_tag):
    ...
    if 'codet5_small' in model_tag:
        bs = 32
        if task == 'summarize' or task == 'translate' or (task == 'refine' and sub_task == 'small') or task == 'compilation' or task == 'decompilation':
            bs = 64
    else:
        # codet5_base
        bs = 28
        if task == 'translate' or task == 'compilation' or task == 'decompilation':
            bs = 25
        elif task == 'summarize':
            bs = 40
    return bs
            ...
            if task == 'summarize':
                ...
                if split_tag == 'test':
                    if task in ['summarize', 'search']:
                        cur_lang = sub_task
                    elif task in ['refine', 'concode', 'clone', 'decompilation']:
                        cur_lang = 'java'
                    elif task == 'defect':
                        cur_lang = 'c'
                    elif task == 'translate':
