...
def add_lang_by_task(target_str, task, sub_task):
    if task == 'summarize':
        ...
        target_str = '<bytecode> ' + target_str
...
def read_java_raw_examples(filename, data_num):
    ...
    with open(filename, encoding="utf-8") as f:
        for idx, line in enumerate(f):
            ...
            src = js['src'].strip(),
            tgt = js['tgt'].strip()
            #print(f"idx: {idx}, src: {src}, tgt: {tgt}")
            #assert False
            examples.append(
                Example(
                    idx=idx,
...
def read_java_compiled_examples(filename, data_num):
    """Read examples from filename."""
    examples = []
    with open(filename, encoding="utf-8") as f:
        for idx, line in enumerate(f):
